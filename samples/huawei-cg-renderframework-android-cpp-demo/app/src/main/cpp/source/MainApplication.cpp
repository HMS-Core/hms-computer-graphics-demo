...
void MainApplication::UnInitialize()
{
    ...
    if (m_lightObject != nullptr) {
        gSceneManager.DeleteObject(m_lightObject);
...
void MainApplication::InitScene()
{
    ...
    m_lightObject = CG_NEW SceneObject(nullptr);
    if (m_lightObject != nullptr) {
        Light* lightCom = m_lightObject->AddComponent<Light>();
        if (lightCom != nullptr) {
...
void MainApplication::ProcessInputEvent(const InputEvent* inputEvent)
{
    ...
    static bool singleTapDetected = false;
...
SceneObject* MainApplication::CreateSkybox()
{
    ...
    const Mesh* mesh = model->GetMesh();
    if (mesh == nullptr) {
        LOGERROR("Failed to get mesh from model.");
